    ...
    <div class="card">
      ...
      <div v-if="boss?.state === 'active'" class="space-y-4">
        ...
        <div class="flex items-center space-x-4">
          ...
          <button
            @click="dealDamage"
            :disabled="!damageAmount || damageAmount <= 0 || !user?.energy"
            class="btn-primary"
          >
            Нанести урон
          </button>
        ...
        <div class="text-sm text-gray-400">
          <p>У вас {{ user?.energy }} энергии</p>
          <p>Максимальный урон: {{ user?.energy * 10 }}</p>
        </div>
      </div>
...
// Нанесение урона
const dealDamage = () => {
  if (!socket || !damageAmount.value) return;
  ...
  battleLog.value.push({
    timestamp: Date.now(),
    message: `Вы нанесли ${damageAmount.value} урона`
  });
};
...
const connectSocket = () => {
  ...
  socket.on('bossUpdate', data => {
    if (boss.value) {
      ...
      boss.value.participants = data.participants || boss.value.participants;
    }
